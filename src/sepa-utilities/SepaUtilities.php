...
class SepaUtilities
{
    ...
    public static function toUpperWithoutWhitespaces(string $input): string
    {
    ...
    /**
     * Checks the structured creditor reference ISO 11649 is valid
     * https://www.mobilefish.com/services/creditor_reference/creditor_reference.php
     *
     * @param string $creditorReference
     * @return string|false The valid creditor reference or false if it is not valid
     */
    public static function checkCreditorReference(string $creditorReference): string|bool
    {
        $creditorReference = self::toUpperWithoutWhitespaces($creditorReference);

        if(!self::checkLength($creditorReference, 25) OR 
           !preg_match('/^RF\d{2}/', $creditorReference) OR 
           !preg_match('/^RF[0-9A-Z]*$/', $creditorReference)) {
            return false;
        }

        $checkDigit = substr($creditorReference, 2, 2); 
        $reference = substr($creditorReference, 4); 

        $toCheck = $reference . 'RF00'; // Referenz + "RF" + "00" 
        $numericString = str_replace(self::ALPHABET, self::ALPHABET_VALUES, $toCheck);
        // Modulo 97 
        $remainder = bcmod($numericString, '97');
        $calculatedCheckDigit = str_pad(bcsub('98', $remainder), 2, '0', STR_PAD_LEFT);

        return ($calculatedCheckDigit === $checkDigit) ? $creditorReference : false;
    }
     ...
     *                        'ultmtcdtr','ultmtdbtr','rmtinf','cdtrrefinf','orgnldbtracct_iban','iban','bic',
    ...
    public static function check(string $field, mixed $input, ?array $options = null): mixed
    {
        ...
        switch($field)      // fall-through's are on purpose
        {
            ...
            case 'cdtrrefinf':
                return self::checkCreditorReference($input);
